...
def generate_id_card(person_name: str, department: str = "AI Engineering", role: str = "Member", enrolled_date: str = "2026-08-10"):
    """Generate printable official Smart ID Card PDF for an enrolled person."""
    ...
    c.setFillColor(HexColor("#140f00"))
    c.rect(0, 0, 3.5 * inch, 2.25 * inch, fill=1, stroke=0)

    # Top Cyberpunk Gold Header Accent
    c.setFillColor(HexColor("#f59e0b"))
    c.rect(0, 2.05 * inch, 3.5 * inch, 0.20 * inch, fill=1, stroke=0)

    c.setFillColor(HexColor("#000000"))
    c.setFont("Helvetica-Bold", 8)
    c.drawString(0.15 * inch, 2.10 * inch, "VISIONTRACK AI — SMART ID BADGE")

    # Person Name & Details
    c.setFillColor(HexColor("#fbbf24"))
    c.setFont("Helvetica-Bold", 12)
    c.drawString(0.20 * inch, 1.70 * inch, person_name.upper())

    c.setFillColor(HexColor("#fef08a"))
    c.setFont("Helvetica", 9)
    c.drawString(0.20 * inch, 1.45 * inch, f"Dept: {department}")
    c.drawString(0.20 * inch, 1.25 * inch, f"Role: {role}")
    c.drawString(0.20 * inch, 1.05 * inch, f"Enrolled: {enrolled_date}")

    c.setFillColor(HexColor("#f59e0b"))
    c.setFont("Helvetica-Bold", 7)
    c.drawString(0.20 * inch, 0.70 * inch, "VERIFIED 512-D ARCFACE EMBEDDING")
    ...
    d.add(qr_code)
    d.drawOn(c, 2.70 * inch, 0.70 * inch)
